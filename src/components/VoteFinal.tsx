		...
		<section class='flex flex-col'>
			...
			{(result === RESULT_STATUS.IDLE || isLoading) && (
				<>
					...
					<div
						class={`${isLoading ? "opacity-50" : ""} transition grid grid-cols-2 md:grid-cols-6 gap-2 p-4`}>
					...
					<div class='flex justify-center items-center w-full py-10 mb-24 flex-col gap-y-4'>
						<button
							class={`${result === RESULT_STATUS.LOADING ? "disabled opacity-90" : ""} lg:text-2xl font-medium no-underline px-5 py-3 rounded-full uppercase text-blue-950 animate-fade-up bg-white hover:scale-125 hover:bg-yellow-300 hover:border-yellow-400 transition`}
